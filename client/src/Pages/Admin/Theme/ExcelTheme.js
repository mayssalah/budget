...
import { getActiveGroupe,getActiveGroupeUsers } from "../../../Redux/groupeReduce";
...
//groupe budget
const [optionGroupeBudget, setOptionGroupeBudget] = React.useState([
  {
    value: "",
    label: "Groupe Budget",
    isDisabled: true,
...
const [groupeBudgetSelect, setGroupeBudgetSelect] = React.useState(0);

  //verif token
  const getNum = useCallback(async (users,gb) => {
   ...
   if(users.length > 0){ users.forEach((e) => {
    ...
    var response = await dispatch(getNumSujet({users:joinuser,annee:anneeLocal,id_groupe_budget:gb.value}));
  ...
  const getGroupe = useCallback(async (annee) => {
    var groupe = await dispatch(getActiveGroupe({type:2,annee:annee}));
  ...
  const getGroupeBudget = useCallback(async (usersbudget,annee) => {
   var joinuser=[]
   if(usersbudget.length > 0){ usersbudget.forEach((e) => {
      joinuser.push(e.value);
    })
  }
    var groupe = await dispatch(getActiveGroupeUsers({type:0,annee:annee,users:joinuser}));
    var entities = groupe.payload;
    var arrayOption = [];
    entities.forEach((e) => {
      arrayOption.push({ value: e.id, label: e.nom });
    });
    setOptionGroupeBudget(arrayOption);
  }, [dispatch]);
  ...
  useEffect(() => {
   ...
   // getNum();
    getTheme();
    getUser();
    getGroupe(anneeLocal);
  }, [getTheme,verifToken,anneeLocal])
    ...
    if(titre !== "" && num.length > 0 && users.length >0){
      dispatch(
        saveTheme({
          ...
          users: users,
          annee:anneeLocal,
          id_groupe:groupeSelect.value,
          id_groupe_budget:groupeBudgetSelect.value
        })
      ).then(()=>{
            ...
            <Form.Check>
                      ...
                      onClick={(value) => {
                        if (value.target.checked) {
                          setUsers(optionsUser);      
                           getGroupeBudget(optionsUser,anneeLocal)
                        } else {
                          setUsers([]);  
                          getGroupeBudget([],anneeLocal)
                        }
                      }}
                    ></Form.Check.Input>
                ...
                <Form.Group id="roleClass">
                  <Select
                    ...
                    onChange={(value) => {
                      setUsers(value);
                      getGroupeBudget(value,anneeLocal)
                    }}
                    options={optionsUser}
                  />
                </Form.Group>
              ...
              <Col className="pr-1" md="6">
                    <Form.Group id="roleClass">
                      ...
                      <Select
                        className="react-select primary"
                        classNamePrefix="react-select"
                        value={groupeBudgetSelect}
                        placeholder="Groupe Budget"
                        onChange={(value) => {
                          setGroupeBudgetSelect(value);
                          getNum(users,value)
                        }}
                        options={optionGroupeBudget}
                      />
                    </Form.Group>
                  </Col>
              ...
              <Col md="12">
                <ReactTable
                  ...
                  columns={[
                    ...
                    {
                      Header: "Groupe Théme",
                      accessor: "themes.groupe_budgets.nom",
                    },
                    {
                      Header: "Total",
                      accessor: "themes.total",
                    },
                    {
                      Header: "Année",
                    ...
                    },
                    {
                      ...
                      Cell: ({ cell }) => (
                        <div className="block_action">
                          ...
                          >
                            Détail <i className="fa fa-plus" id={"idLigneD_" + cell.row.id} /> 
                          </Button>
                        </div>
                      ),
                    },
