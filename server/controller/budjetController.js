          ...
          forecast
            ...
            .then(function async() {
              user_sujet.findOne({
                group: ["id_groupe"],
                attributes: [
                  [sequelize.fn("GROUP_CONCAT", sequelize.col("id")), "idBg"],
                ],
                where: { id_groupe: r1.dataValues.id_groupeb }
              }).then(function async(usersujet) {
                var idBg = usersujet.dataValues.idBg.split(',');
                budjet.findOne({
                  attributes: [
                    [sequelize.fn("GROUP_CONCAT", sequelize.col("id")), "idBudget"],
                  ],
                  where: { id_user_sujet: idBg }
                }).then(function async(allbudget) {
                  var idBudget = allbudget.dataValues.idBudget.split(',');
                  ligne_budjet.destroy({ where: { id_budjet: idBudget } })
                    .then(function async() {
                      budjet.destroy({ where: { id_user_sujet: idBg } }).then(function async() {
                        user_sujet.destroy({ where: { id: idBg } })
                        groupe.update({
                          total: 0,
                          etat: 0
                        }, { where: { id: id } });
